...
class GoogleScholar(CrawlSpider):
    ...
    rules = (
        ...
        Rule( # Regla de movimiento HORIZONTAL hacia las paginaciones
            LinkExtractor(
                restrict_xpaths='.//div[@id="gs_nml"]',
                allow=r'\?start=' # Si la URL contiene este patron, haz un requerimiento a esa URL
            ), follow=True, callback="parse_start_url"), # El callback es el nombre de la funcion que se va a llamar con la respuesta al requerimiento hacia estas URLs
    ...
    def parse_start_url(self, response):
        ...
        for articulo in articulos:
            ...
            pagina = sel.xpath('//b[@class="gs_nma"]/text()')

            # Llenamos el Item
            print('Pagina:', pagina)
            item.add_value('pagina', pagina)
